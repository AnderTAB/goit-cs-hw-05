...
if __name__ == '__main__':
    ...
    result = asyncio.run(map_reduce(url)) #Всі слова підряд

    filtered_result = {key: value for key, value in result.items() if key not in conjunctions and len(key) > 3}  #приберем сполучники та допоміжні слова
    print(filtered_result) 
    visualize_top_words(filtered_result)